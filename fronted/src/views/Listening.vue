...
async function startQuiz(mode) {
  ...
  else { const s = await practiceStore.startSession('listening', quizId); router.push(`/quiz/${s?.quiz?.id || quizId}?mode=practice`) }
}
onMounted(async () => { loading.value = true; try { items.value = await listMockTests({ skillId: 2 }) } finally { loading.value = false } })
</script>
