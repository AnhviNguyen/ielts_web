    ...
    <section class="section-white section-compact">
      <div class="app-container">
        <div class="page-header page-header--row" data-tour="page-header">
          <div>
            <h1 class="font-display">Speaking</h1>
            <p class="page-subtitle">{{ items.length }} bộ đề · IELTS Speaking</p>
          </div>
          <div class="ml-auto">
        ...
        <template v-else>
          <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3" data-tour="test-grid">
            ...
            />
            <p v-if="!paged.length" class="col-span-3 py-16 text-center text-[var(--text-subdued)]">
...
const { completedIds } = useCompletedQuizIds('speaking')
...
async function loadItems() {
  ...
  loadError.value = ''
  try {
    items.value = await listMockTests({ skillId: 8 })
  } catch (e) {
...
async function startQuiz(mode) {
  ...
  if (!quizId) return
  const s = await practiceStore.startSession('speaking', quizId)
